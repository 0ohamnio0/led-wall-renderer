...
Adapted for LED rendering module.
*/
...
function getWebGLContext () {
  ...
  const isWebGL2 = !!_gl
  if (!isWebGL2)
    _gl = canvas.getContext('webgl', params) || canvas.getContext('experimental-webgl', params)
  ...
  return { gl: _gl, ext: { formatRGBA, formatRG, formatR, halfFloatTexType, supportLinearFiltering } }
}
...
function getSupportedFormat (_gl, internalFormat, format, type) {
  if (!supportRenderTextureFormat(_gl, internalFormat, format, type)) {
    switch (internalFormat) {
      case _gl.R16F: return getSupportedFormat(_gl, _gl.RG16F, _gl.RG, type)
      case _gl.RG16F: return getSupportedFormat(_gl, _gl.RGBA16F, _gl.RGBA, type)
      default: return null
    }
  }
...
function initSunraysFramebuffers () {
  ...
  const texType = ext.halfFloatTexType, r = ext.formatR
  const filtering = ext.supportLinearFiltering ? gl.LINEAR : gl.NEAREST
...
function generateColor () {
  if (config.WHITE_ONLY) return { r: 0.15, g: 0.15, b: 0.15 }
...
function getResolution (resolution) {
  ...
  let min = Math.round(resolution), max = Math.round(resolution * aspectRatio)
...
function applyBloom (source, destination) {
  ...
  for (let i = bloomFramebuffers.length - 2; i >= 0; i--) {
    ...
    gl.uniform2f(bloomBlurProgram.uniforms.texelSize, last.texelSizeX, last.texelSizeY)
    gl.uniform1i(bloomBlurProgram.uniforms.uTexture, last.attach(0))
    gl.viewport(0, 0, baseTex.width, baseTex.height); blit(baseTex); last = baseTex
  }
  gl.disable(gl.BLEND)
...
function blur (target, temp, iterations) {
  ...
  for (let i = 0; i < iterations; i++) {
    ...
    gl.uniform1i(blurProgram.uniforms.uTexture, target.attach(0)); blit(temp)
    gl.uniform2f(blurProgram.uniforms.texelSize, 0.0, target.texelSizeY)
    gl.uniform1i(blurProgram.uniforms.uTexture, temp.attach(0)); blit(target)
  }
}
...
// ─── GUI ───
const gui = new GUI({ title: 'Fluid Sim — LED' })
gui.add(config, 'DYE_RESOLUTION', { 'high': 1024, 'medium': 512, 'low': 256 }).name('quality').onFinishChange(initFramebuffers)
...
gui.add(config, 'WHITE_ONLY').name('white only')
gui.add(config, 'AUTO_SPLAT_INTERVAL', 0.5, 10, 0.5).name('auto splat (sec)')
gui.add({ fun: () => { splatStack.push(Math.floor(Math.random() * 20) + 5) } }, 'fun').name('Random splats')
...
gui.addColor(config, 'BACK_COLOR').name('background')
...
export { canvas, start, stop, config, gui }
